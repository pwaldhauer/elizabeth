...
var movesApi = require('./MovesApi').MovesApi;
...
Elizabeth.prototype.startInit = function startInit() {
	...
	if(fs.existsSync(configFilePath)) {
		console.log('> Found a config.json. If you want to recreate it, please delete it first.');
		process.exit(1);
	}
	...
	console.log('https://dev.moves-app.com/clients');
...
Elizabeth.prototype.startExport = function startExport() {
	// parse date range, default is last 7 days plaintext
	...
	console.log('> Hehe, ' + days + ' days!');
	for(var i = 0; i < days; i++) {
		var day = range.start.add('days', 1).format('YYYYMMDD');

		moves.getStoryline(day, function(err, result) {
			if(err) {
				console.log('> WHAT? Error: ', err);
				process.exit(1);
			}

			exportPlugin.exportDay(result, function(result) {
...
function _getDateRangeFromParameters(range, params) {
	if(params.dayStart) {
		range.start = moment(params.dayStart, dateFormat);
	}

	if(params.dayEnd) {
		range.end = moment(params.dayEnd, dateFormat);
	}

	if(params.days) {
		range.start = moment().subtract('days', params.days);
		range.end = moment();
	} 
