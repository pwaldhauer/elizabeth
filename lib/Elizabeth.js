...
function _exportInterval(exportPlugin) {
  return function(err, result) {
    if (err) {
      console.log('> Error: ', err);
      process.exit(1);
...
Elizabeth.prototype.startExport = function startExport() {
  ...
  var resolution = (typeof exportPlugin.options.resolution === 'undefined') ? "day" : exportPlugin.options.resolution;

  console.log('> Using ' + exportPlugin.help.name + ' to export ' + days + ' days with trackPoints ' + trackPoints + '!');
  console.log('');

  if (resolution == "interval") {
    var from = range.start.format('YYYYMMDD');
    var to = range.end.format('YYYYMMDD');
    moves.getStoryline(
        {from: from,
          to: to,
          trackPoints: trackPoints},
          _exportInterval(exportPlugin))
  } else {
...
Elizabeth.prototype.printHelp = function printHelp() {
  ...
  console.log(' --currentMonth - Export current calendar month until today');
  console.log('');
...
function _getDateRangeFromParameters(range, params) {
  ...
  if(params.lastMonth) {
		range.start = moment().subtract(1,'months').date(1);
    range.end = moment().subtract(1,'months').endOf('month');
  }

  if(params.currentMonth) {
		range.start = moment().date(1);
    range.end = moment();
  }
